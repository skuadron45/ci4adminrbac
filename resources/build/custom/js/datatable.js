var DTHELPER = (function($) {
  ...
  dtHelper.deleteButton = function(idParam, tableName) {
    return HELPER.Html.buttonTag(
      tableName + ".onDelete('" + idParam + "')",
      "btn btn-xs text-danger",
      "Delete",
  ...
  dtHelper.editButton = function(idParam, tableName) {
    return HELPER.Html.buttonTag(
      tableName + ".onEdit('" + idParam + "')",
      "btn btn-xs text-info",
      "Edit",
  ...
  dtHelper.getActionHref = function(editLink, deleteLink) {
    return {
      ...
      render: function(data, type, row, meta) {
        var html = HELPER.Html.aHref(
          editLink + data,
          "btn btn-xs text-info",
          "Edit",
          '<i class="fa fa-pencil-alt"></i>'
        );
        html += HELPER.Html.aHref(
          deleteLink + data,
          "btn btn-xs text-danger",
          "Delete",
          '<i class="fas fa-times-circle"></i>'
  ...
  this.buildButtons = function() {
    ...
    options.addModal &&
      (buttons +=
        ...
        name +
        '_FORM.onShow()" class="btn btn-success btn-sm" title="Add"><i class="fas fa-plus-circle"></i></button>');
...
(function() {
  ...
  var isEditState = function() {    
    return state == "edit";
  };

  this.renderField = function(field) {
    var isEdit = isEditState();
    var type = field.type;
    ...
    if (isEdit) {
        placeholder = field.placeholderEdit ? field.placeholderEdit : "";
    }    
    ...
    switch (type) {
      ...
      case "textarea":
        html +=
          '<textarea rows="5" class="form-control form-control-sm" id="' +
          fieldId +
          '" name="' +
          fieldName +
          '" placeholder="' +
